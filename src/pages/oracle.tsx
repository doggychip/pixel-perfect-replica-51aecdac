...
const regimeIcon: Record<string, any> = {
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border text-muted-foreground text-left">
            ...
            <th className="py-2 px-2 font-medium">Instrument</th>
            <th className="py-2 px-2 font-medium">Price</th>
            <th className="py-2 px-2 font-medium">Change</th>
            <th className="py-2 px-2 font-medium">Regime</th>
            <th className="py-2 px-2 font-medium">Top Pattern</th>
            <th className="py-2 px-2 font-medium">Signal</th>
            <th className="py-2 px-2 font-medium">Collisions</th>
          </tr>
        </thead>
        <tbody>
          {results.map((r, i) => {
            ...
            return (
              <tr key={r.instrument} className="border-b border-border/50 hover:bg-muted/20">
                <td className="py-2 px-2 text-muted-foreground">{i + 1}</td>
                <td className="py-2 px-2 font-medium">{r.instrument.replace("_USDT", "")}</td>
                <td className="py-2 px-2">{formatPrice(r.price)}</td>
                <td className={`py-2 px-2 ${r.change_pct >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                  {formatPct(r.change_pct)}
                </td>
                <td className="py-2 px-2">
                  <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs border ${regimeBg[r.regime] || ""} ${regimeColor[r.regime] || ""}`}>
                    <Icon className="w-3 h-3" />
                    {regimeLabel[r.regime] || r.regime}
                  </div>
                </td>
                <td className="py-2 px-2 text-xs">{r.top_pattern || "-"}</td>
                <td className="py-2 px-2">{strengthBar(r.signal_score)}</td>
                <td className="py-2 px-2">
                  {r.collision_count > 0 ? (
                    <Badge variant="outline" className="text-cyan-400 border-cyan-500/30">{r.collision_count}</Badge>
                  ) : "-"}
                </td>
              </tr>
            );
    ...
    <div className="p-3 rounded-lg bg-muted/20 border border-border/50 space-y-2">
      ...
      {ci.bridge_theory !== "(rule-template)" && (
        <p className="text-xs text-muted-foreground">
          Bridge: {ci.bridge_theory.replace(/_/g, " ")} ({ci.bridge_domain})
        </p>
      )}
      <p className="text-xs text-foreground/80">{ci.trading_rule}</p>
    </div>
  );
}
...
function DetailPanel({ instrument }: { instrument: string }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold">{data.instrument.replace("_USDT", "")}/USDT</h3>
          <p className="text-2xl font-mono font-bold">{formatPrice(data.price)}</p>
        </div>
        <div className="text-right space-y-1">
          <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs border ${regimeBg[data.regime] || ""} ${regimeColor[data.regime] || ""}`}>
            <Icon className="w-3 h-3" />
            {regimeLabel[data.regime]}
          </div>
          <p className={`text-sm ${data.change_pct >= 0 ? "text-emerald-400" : "text-red-400"}`}>
      ...
      {data.patterns.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-sm font-semibold text-muted-foreground">Detected Patterns</h4>
      ...
      {data.collision_insights.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-sm font-semibold text-muted-foreground">Collision Insights</h4>
      ...
      {data.theory_details.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-sm font-semibold text-muted-foreground">Theory Details</h4>
          {data.theory_details.slice(0, 3).map((t, i) => (
            <div key={i} className="p-3 rounded-lg bg-muted/20 border border-border/50 space-y-1">
              ...
              {t.equation && <p className="text-xs font-mono text-muted-foreground">{t.equation.slice(0, 80)}</p>}
...
export default function OraclePage() {
  ...
  const { data: statsData } = useQuery<{ theories: number; collisions: number }>({
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Card><CardContent className="p-4">
          <p className="text-xs text-muted-foreground">Theories</p>
          <p className="text-2xl font-bold">{statsData?.theories ?? "-"}</p>
        </CardContent></Card>
        <Card><CardContent className="p-4">
          <p className="text-xs text-muted-foreground">Collisions</p>
          <p className="text-2xl font-bold">{statsData?.collisions ?? "-"}</p>
        </CardContent></Card>
        <Card><CardContent className="p-4">
      ...
      {Object.keys(regimeCounts).length > 0 && (
        <div className="flex flex-wrap gap-2">
          {Object.entries(regimeCounts).map(([regime, count]) => {
            ...
            return (
              <div key={regime} className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs border ${regimeBg[regime] || ""} ${regimeColor[regime] || ""}`}>
                <Icon className="w-3 h-3" />
                {regimeLabel[regime]}
        ...
        <div className="lg:col-span-2">
          <Card>
            <CardContent className="p-4 space-y-3">
              <h2 className="text-sm font-semibold text-muted-foreground">Market Scan (ranked by signal)</h2>
              {scanLoading ? (
                ...
                <p className="text-sm text-muted-foreground py-8 text-center">No scan data. Click Scan to start.</p>
              )}
            </CardContent>
          </Card>
        </div>
      ...
      {transitions.length > 0 && (
        <Card>
          <CardContent className="p-4 space-y-3">
            <h2 className="text-sm font-semibold text-muted-foreground">Regime Transitions</h2>
            <div className="space-y-2">
              {transitions.map((t, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-3 text-sm">
                    <span className="font-medium w-16">{t.instrument.replace("_USDT", "")}</span>
                    <div className={`inline-flex items-center gap-1 ${regimeColor[t.from_regime] || ""}`}>
                      <FromIcon className="w-3 h-3" />
                      {regimeLabel[t.from_regime]}
                    </div>
                    <span className="text-muted-foreground">→</span>
                    <div className={`inline-flex items-center gap-1 ${regimeColor[t.to_regime] || ""}`}>
                      <ToIcon className="w-3 h-3" />
                      {regimeLabel[t.to_regime]}
                    </div>
                    <span className="text-muted-foreground ml-auto">{formatPrice(t.price)}</span>
                  </div>
                );
              })}
            </div>
